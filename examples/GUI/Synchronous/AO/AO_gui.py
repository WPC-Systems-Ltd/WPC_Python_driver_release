...
##  This is example for Analog Output with WPC DAQ Device with synchronous mode.
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def connectEvent(self):
        ...
        ## Open AO port
        status = self.dev.AO_open(self.port)
        print("AO_open status: ", status)
    ...
    def disconnectEvent(self):
        ...
        ## Close AO port
        status = self.dev.AO_close(self.port)
        print("AO_close status: ", status)
    ...
    def writeAOValue(self):
        ...
        status = self.dev.AO_writeAllChannels(self.port, voltage_list)
