'''
AHRS - AHRS_getOrientation.py with synchronous mode.

This example demonstrates the process of getting AHRS three axis orientation data from WifiDAQE3AO.

To begin with, it demonstrates the steps to open the AHRS and configure the AHRS parameters.
Next, it outlines the procedure for the AHRS orientation data.
...
def main():
    ...
    try:
        ...
        while True:
            data = dev.AHRS_getOrientation(port, timeout)
            print(data)
