'''
Drone - Drone_simple_rotate_yaw.py with synchronous mode.
...
def main():
    ...
    try:
        ...
        ## Yaw rotate with NED frame
        err = dev.Drone_moveNedRelYaw(rotate_angle, velocity, timeout)
        print(f"Drone_moveNedRelYaw, status: {err}")

        ## Read inposition
        inposition = 0
        while inposition == 0:
            posi_list = dev.Drone_readInposition(timeout)
            yaw_ready = posi_list[2]
            inposition = int(bool(yaw_ready))
            if inposition == 1:
                print("Reached yaw!")
