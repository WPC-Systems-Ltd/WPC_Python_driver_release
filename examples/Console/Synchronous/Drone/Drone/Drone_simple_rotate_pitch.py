'''
Drone - Drone_simple_rotate_pitch.py with synchronous mode.
...
def main():
    ...
    try:
        ...
        ## Pitch rotate with vehicle frame
        err = dev.Drone_moveVehicleAbsPitch(rotate_angle, velocity, timeout)
        print(f"Drone_moveVehicleAbsPitch, status: {err}")

        ## Read inposition
        inposition = 0
        while inposition == 0:
            posi_list = dev.Drone_readInposition(timeout)
            pitch_ready = posi_list[1]
            inposition = int(bool(pitch_ready))
            if inposition == 1:
                print("Reached pitch!")
