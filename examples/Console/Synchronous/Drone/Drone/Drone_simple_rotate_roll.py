'''
Drone - Drone_simple_rotate_roll.py with synchronous mode.
...
def main():
    ...
    try:
        ...
        ## Roll rotate with vehicle frame
        err = dev.Drone_moveVehicleAbsRoll(rotate_angle, velocity, timeout)
        print(f"Drone_moveVehicleAbsRoll, status: {err}")

        ## Read inposition
        inposition = 0
        while inposition == 0:
            posi_list = dev.Drone_readInposition(timeout)
            roll_ready = posi_list[0]
            inposition = int(bool(roll_ready))
            if inposition == 1:
                print("Reached roll!")
