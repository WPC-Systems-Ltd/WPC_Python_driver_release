...
async def main():
    ...
    try:
        ...
        ## Motion configure
        err = dev.Motion_cfgCircularInterpo(port, x_axis, y_axis, center_point_x, center_point_y, finish_point_x, finish_point_y, circular_dir_cw, speed = 1000)
        ...
        err = dev.Motion_cfgJerkAndAccelMode(port, axis, jerk, scurve)
        ...
        for i in range(4):
            err = dev.Motion_enableServoOn(port, i, int(True))
        ...
        move_status = 0; 
        while move_status == 0:
            x_axis_move_status = dev.Motion_getMoveStatus(port, x_axis)
            y_axis_move_status = dev.Motion_getMoveStatus(port, y_axis)
