'''
Motion - Motion_1axis_move_with_breakpoint.py with synchronous mode.
...
def main():
    ...
    try:
        ...
        print(f"Motion_cfgBreakPoint in axis{axis}, status: {err}")

        err = dev.Motion_enableBreakPoint(port, axis, int(True), timeout)
        print(f"Motion_enableBreakPoint in axis{axis}, status: {err}")
