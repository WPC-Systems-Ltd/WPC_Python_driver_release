'''
Motion - Motion_1axis_move_with_breakpoint.py with asynchronous mode.
...
async def main():
    ...
    try:
        ...
        print(f"cfgBreakPoint_async in axis{axis}, status: {err}")

        err = await dev.Motion_enableBreakPoint_async(port, axis, int(True))
        print(f"enableBreakPoint_async in axis{axis}, status: {err}")
