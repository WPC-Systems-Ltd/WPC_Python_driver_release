...
async def main():
    ...
    try:
        ...
        print(f"cfgCircularInterpo_async in port {port}, status: {err}")

        err = await dev.Motion_startCircularInterpo_async(port)
        print(f"startCircularInterpo_async in axis{axis}, status: {err}")
        ...
        print(f"cfgJerkAndAccelMode_async in axis{axis}, status: {err}")
        ...
        for i in range(4):
            ...
            print(f"enableServoOn_async in axis{i}, status: {err}")
        ...
        for i in range(4):
            ...
            print(f"enableServoOff_async in axis{i}, status: {err}")
