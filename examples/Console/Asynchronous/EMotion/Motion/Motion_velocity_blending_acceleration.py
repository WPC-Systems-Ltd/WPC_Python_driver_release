...
async def main():
    ...
    try:
        ...
        err = await dev.Motion_cfgAxisMove_async(port, axis, velocity_mode, target_posi=5000, velo=3000, accel=100000, decel=100000)
        ...
        print(f"overrideAxisVelocity_async in axis{axis}, status: {err}")

        ## Motion override acceleration
        err = await dev.Motion_overrideAxisAccel_async(port, axis, new_accel, new_decel)
        print(f"overrideAxisAccel_async in axis{axis}, status: {err}")
