'''
Motion - Motion_1axis_move_with_alarm_in.py
...
async def main():
    ...
    try:
        ...
        print("cfgAxisParam_async:", err)
            
        err = await dev.Motion_cfgLimit_async(port, axis, int(True), int(True), active_high)
        print("cfgLimit_async:", err)

        err = await dev.Motion_cfgAlarm_async(port, axis, int(False), active_low)
        print("cfgAlarm_async:", err)
