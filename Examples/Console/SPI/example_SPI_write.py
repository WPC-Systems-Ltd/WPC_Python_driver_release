...
async def main():
    ...
    try: 
        ...
        '''
        Read default value from address 0x01
        '''
        ## Set pin0 to low
        status = await dev.DO_writeValuePins(DO_port, DO_index, [0]) 
        if status == 0: print("DO_writeValuePins: OK")
       
        data = await dev.SPI_readAndWrite(SPI_port, [0x01]*2)
        data = ['{:02x}'.format(value) for value in data]
        print("read data :", data) ## 0x03 for 1 sample and K type
        ...
        '''
        Now, set 4 samples and J type to address 0x81
        '''
        ...
        status = await dev.SPI_write(SPI_port, [chr(0x81), chr(0x22)])
        ...
        '''
        So, it must read 0x22 from address 0x01
        '''
